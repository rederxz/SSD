...
def hard_negative_mining(idxes, outputs_cls, targets_cls, num_classes_without_bgd, max_num):
    # softmax
...
class SSDLoss:
    ...
    def __init__(self, alpha, num_classes_without_bgd):
        self.alpha = alpha
    ...
    def __call__(self, outputs, targets):
        ...
        positive_loss_cls = tf.math.exp(positive_outputs_cls) / tf.reduce_sum(tf.math.exp(positive_outputs_cls), axis=-1)
        ...
        hard_negative_loss_cls = hard_negative_mining(negative_anchor_idxes, negative_outputs_cls,
                                                      negative_targets_cls, 3*num_positive_anchors)  # positive : negative >= 1 :  3
        ...
        loss = loss_cls + self.alpha * loss_loc
